            ...
            apps = {
              provision = {
                ...
                program =
                  ...
                  in
                  "${pkgs.writeScript "provision-${cloudProvider}" ''
                    #!/bin/sh
                    set -e
                    mkdir -p tf
                    ...
                    ${pkgs.terraform}/bin/terraform apply -auto-approve \
                      ${lib.concatStringsSep " " (
                        lib.mapAttrsToList (n: v: "-var '${n}=${v}'") credentials.${cloudProvider}
                      )}
                    ...
                    for ip in $VM_IPS; do
                      ${pkgs.nixos-anywhere}/bin/nixos-anywhere \
                        --flake .#${hostnamePrefix}''${index} \  # Fix: Use shell variable escaping
                        root@$ip
                      index=$((index + 1))
                    done
                  ''}";
